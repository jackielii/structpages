...
// Test page types for ComponentSelection
...
// Props that uses ComponentSelection to return different data
func (selectionTestPage) Props(r *http.Request, sel *ComponentSelection) (string, error) {
	switch {
	case sel.Selected(selectionTestPage.TodoList):
		return "todo data", nil
	case sel.Selected(selectionTestPage.Content):
		return "content data", nil
	case sel.Selected(selectionTestPage.Page):
...
func TestComponentSelection_Selected(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The body contains the data that Props returned based on ComponentSelection
			// This proves that Props correctly used ComponentSelection.Selected()
		})
	}
}

func TestComponentSelection_DifferentReceiverTypes(t *testing.T) {
	// Test that Selected() correctly distinguishes between methods on different types
	// even if they have the same name

	// Create ComponentSelection for selectionTestPage.TodoList
	pageType := reflect.TypeOf(selectionTestPage{})
	method, _ := pageType.MethodByName("TodoList")
	sel := &ComponentSelection{selectedMethod: method}

	// Should match selectionTestPage.TodoList
	if !sel.Selected(selectionTestPage.TodoList) {
		t.Error("Should match selectionTestPage.TodoList")
	}

	// Should NOT match anotherPage.TodoList (different receiver type)
	if sel.Selected(anotherPage.TodoList) {
		t.Error("Should NOT match anotherPage.TodoList (different type)")
	}
}

func TestComponentSelection_InvalidMethodExpression(t *testing.T) {
	pageType := reflect.TypeOf(selectionTestPage{})
	method, _ := pageType.MethodByName("Page")
	sel := &ComponentSelection{selectedMethod: method}

	// Test with invalid inputs
	if sel.Selected("not a method") {
