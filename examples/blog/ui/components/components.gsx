...
component Alert(kind AlertKind, msg string) {
	{ if msg != "" {
		<div class={ "rounded border px-3 py-2 text-sm", alertClasses(kind) }>{msg}</div>
	} }
}
	...
	<label class="block text-sm">
		...
		<input
			...
			class={
				...
				"border-slate-300": errMsg == "",
				"border-red-400": errMsg != "",
			}
		/>
		{ if errMsg != "" {
...
component Pagination(p PageNav) {
	<nav id="pagination" class="flex items-center justify-between text-sm">
		...
		<div class="flex gap-2">
			{ if p.HasPrev() {
				<a class="rounded border px-2 py-1 hover:bg-slate-50" href={ p.URL(p.Page - 1) }>← Prev</a>
			} else {
				<span class="rounded border px-2 py-1 text-slate-300">← Prev</span>
			} }
			<span class="px-2 py-1 text-slate-600">Page {p.Page} of {p.Pages()}</span>
			{ if p.HasNext() {
				<a class="rounded border px-2 py-1 hover:bg-slate-50" href={ p.URL(p.Page + 1) }>Next →</a>
			} else {
				<span class="rounded border px-2 py-1 text-slate-300">Next →</span>
			} }
		</div>
	</nav>
}
...
component ErrorBlock(status int, msg string) {
	<div class="rounded-lg border border-red-200 bg-red-50 p-6 text-center">
		...
		<a class="mt-4 inline-block text-sm text-red-700 underline" href="/">Back to home</a>
	</div>
}
