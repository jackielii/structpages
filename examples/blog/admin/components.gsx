			...
			<ul class="divide-y text-sm">
				...
				{ for _, p := range posts {
					<li class="flex items-center justify-between py-2">
						...
						{ if p.Published {
							<span class="rounded bg-emerald-100 px-2 py-0.5 text-xs text-emerald-800">live</span>
						} else {
							<span class="rounded bg-slate-200 px-2 py-0.5 text-xs text-slate-700">draft</span>
						} }
					</li>
				} }
			</ul>
			...
			<tbody>
				{ for _, p := range posts {
					<tr class="border-t">
						...
						<td class="px-3 py-2">
							{ if p.Published {
								<span class="rounded bg-emerald-100 px-2 py-0.5 text-xs text-emerald-800">published</span>
							} else {
								<span class="rounded bg-slate-200 px-2 py-0.5 text-xs text-slate-700">draft</span>
							} }
						</td>
						<td class="px-3 py-2 text-slate-500">{p.CreatedAt.Format("Jan 2, 2006")}</td>
				...
				{ if len(posts) == 0 {
					<tr><td colspan="4" class="px-3 py-6 text-center text-slate-500">No posts yet.</td></tr>
				} }
			</tbody>
		</table>
	</div>
